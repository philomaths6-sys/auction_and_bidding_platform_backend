...
async def place_bid(
    ...
    try:
        bid, auction_info = await bid_service.place_bid(
    ...
    await ws_manager.broadcast(auction_id, {
        ...
        'new_end_time': auction_info['auction_end_time']
    })
    return bid
...
async def my_bids(
    ...
    """Returns all bids placed by currently authenticated user."""
    result = await db.execute(
        select(Bid)
...
@router.get('/my-wins', response_model=list[BidResponse])
async def my_wins(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
